...
\begin{tcolorbox}[
    ...
    title=\textbf{Important Simplification Note}, % Title in the box header
  ...
  The simplified example provided uses decimal digits and an abstract block structure for demonstrative purposes. Please remember that the actual Simple-8b algorithm operates on fixed 64-bit words, where a 4-bit selector and 60 bits of payload are used to pack the resulting \textbf{binary} integers efficiently.
\end{tcolorbox}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=1\textwidth]{images/cloudflare-benchmark-precomputed-aggregations.png}
  \caption{Performance efficiency gained through pre-computed aggregations.}~\label{fig:precomputed_aggregations}
\end{figure}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=1\textwidth]{images/tigerdata-continuous-aggregate.png}
  \caption{TimescaleDB's implementation of continuous aggregations\cite{TimescaleDB:09}}\label{fig:continuous-aggregates}
\end{figure}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{images/cloudflare-benchmark.png}
  \caption{Cloudflare benchmark results: query time improvement after migrating to TimescaleDB continuous aggregates\cite{TimescaleDB:05}}\label{fig:cloudflare-benchmark}
\end{figure}
